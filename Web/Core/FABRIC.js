    ...
    if (isBrowser) {
        ...
        for (globalI = scripts.length - 1; globalI > -1 && (script = scripts[globalI]); globalI--) {
            ...
            */
            ...
            //////////////////////////////////////////////////////
            // PT 31-01-12 Modifications specific to Fabric.
            // We would like to automatically download and install
            // dependencies using the url that the FABRIC file is
            // loaded from.
            // We could specify a baseUrl with a call to configure,
            // however this can be automtically determeined here.
            if (script.getAttribute('src').indexOf('FABRIC.js')) {
                if (!cfg.baseUrl) {
                    //Pull off the directory of the script file 
                    //to use as the baseUrl.
                    src = script.getAttribute('src').split('/');
                    src.pop();
